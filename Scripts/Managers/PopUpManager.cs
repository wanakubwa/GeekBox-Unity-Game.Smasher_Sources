...
public class PopUpManager : ManagerSingletonBase<PopUpManager>
{
    ...
    public WinPopUpController ShowWinUIPopUp(int wallsCounter)
    {
        if(CheckDoublePopUp<WinPopUpController>() == true)
        {
            return GetPopUpController<WinPopUpController>();
        }

        WinPopUpController popUpController = ShowPopUp<WinPopUpController>(WinUIPopUp, false);
        popUpController.SetData(wallsCounter);
        return popUpController;
    }
